...
| A49 | Multi-Setup mit **Werkstück-Transformation** (Rotation/Spiegelung/Wenden zwischen Setups). **Kern fertig (alpha.18):** `cam/umspannung.py` (`WerkstueckTransformation`, `transformiere_punkt/toolpath` mit Bogen-Drehrichtungs-Korrektur bei Spiegelung, `stabilitaets_hinweise`), `Setup.transformation`-Feld, Endpoint `POST /api/operations/transformiere` + MCP `toolpath_transformieren`, UI-Editor pro Setup in WorkflowView (spiegeln/drehung/wenden/Maße). **Auto-Apply fertig (alpha.19):** `transformation`-Option am `POST /api/operations/postprocess` (wird als ERSTER Pipeline-Schritt auf ALLE Toolpaths angewendet, damit Rampen/Fahrweg/Rapid-Safety auf finalen Maschinen-Koordinaten rechnen) + MCP `gcode_erzeugen(transformation=…)` + GCodeEditorView „Werkstück-Lage"-Wähler (zieht Setups-mit-Umspannung, Footgun-Warnung bei Spiegeln ohne Maß). 18 Tests. **Mehrseitig-Assistent fertig (alpha.19):** WorkflowView „⇄ Mehrseitig einrichten" mit 2 Modi — 2-seitig (Spiegeln+Wenden, legt Seite B an) und N-seitig (drehen/indexieren, legt N Setups mit `drehung_grad = i·360/N` + Dreh-Pausen an). **Maschinen-Umbau-Pause** (AchsWechselSchritt) + strukturiertes Spannmittel-Modell + Stabilitäts-Heuristik vorhanden. **Offen:** Rest-Bearbeitung (Heightmap konsumieren, → I6), Frontend-Rest-Viz. | [#44](https://github.com/MadGapun/CAMWOSA/issues/44) | [Workflow-Modul](Workflow-Modul.md) · [Multi-Werkzeug-Setup](Multi-Werkzeug-Setup.md) | 🔶 |
...
| I6 | **Rest-Material-Tracking** zwischen Operationen (Stock = Voxel-Ergebnis vorheriger Pass). Verschraenkt mit A49. **Grundlage fertig (alpha.19):** `cam/rest_material.py` (`rest_heightmap` — höchste verbleibende Material-Z pro XY-Spalte aus dem Voxel-Grid; verkettet mehrere Pässe = Schruppen→Schlichten teilen Stock) + `simuliere_grid`-Refactor in `cam/simulation.py` + Endpoint `POST /api/simulation/rest-heightmap` + MCP `rest_material_heightmap`. 8 Tests. **Offen:** Strategie konsumiert die Heightmap (echte Rest-Bearbeitung), Frontend-2D-Overlay. | [#45](https://github.com/MadGapun/CAMWOSA/issues/45) | [3D-Strategien](3D-Strategien.md) | 🔶 |
...
| P5 | **Programm-Start-Warmlauf** (alpha.20) — optionaler Spindel-Warmlauf am Programmstart (`M3 S<warmlauf_rpm>` + `G4 P<warmlauf_s>`, danach rampt der Erstschnitt auf Schnittdrehzahl). `PostProcessor.warmlauf()` + `PostKontext.warmlauf_s/rpm`, Quelle `Spindel.warmlauf_zeit_s/warmlauf_rpm` (editierbar im Spindel-Editor) oder Body-Override am `/postprocess`. UI-Checkbox im G-Code-Editor, MCP `gcode_erzeugen(warmlauf=…)`. Beide Werte > 0 nötig, Default aus. Bildet Markus' manuellen 10-s-Warmlauf optional ab. 4 Tests. | [#54](https://github.com/MadGapun/CAMWOSA/issues/54) | [Postprozessor-GRBL](Postprozessor-GRBL.md) | ✅ |
...
| M7 | **Getrennte G-Code-Dateien bei Umbau/Umkabeln** (Markus 2026-06-05) — eine `M0`-Pause setzt voraus, dass die Maschine **an + verbunden** bleibt. Beim **Umkabeln** (Rotary-Remap, Spindel-Umverdrahtung) muss die Maschine **aus** → Streaming-Verbindung reißt ab → eine Einzeldatei mit Pause funktioniert nicht. Lösung: wählbares `getrennte_datei`-Flag an Umspann-/Achswechsel-/Pause-Schritten → Export endet die Datei dort + Abschluss-Hinweis „Maschine ausschalten, umkabeln, nächste Datei laden". **Increment 1 (alpha.21):** Modell (`getrennte_datei` auf UmspannSchritt/PauseSchritt/AchsWechselSchritt + SetupPause; AchsWechsel default an) + `gliedere_schritte_in_bloecke`-Split + Abschluss-Hinweis + PauseEditor-Checkbox + Tests. **Increment 2 (offen):** echter Multi-Datei-Download-Export im UI (schließt die flat-Store↔Per-Setup-Lücke, gemeinsam mit A49). Passt zum „kein direkter Sender-Push"-Prinzip — getrennte Dateien sind genau der dateibasierte Übergabepunkt. | [#44](https://github.com/MadGapun/CAMWOSA/issues/44) | 🔶 |
...
| Q3 | **Multi-Geometrie pro Operation + Per-Geometrie-Override** — eine Operation bearbeitet jetzt **alle** zugewiesenen Geometrien (vorher nur die erste via `waehleGeometrie`), je Geometrie ein Toolpath, zu einem kombiniert (`api/toolpathKombi.ts`). Pro Kontur optional Feed/Max-Tiefe/Plunge überschreibbar (`geometrie_overrides`, UI `GeometrieFeinabstimmung` ab 2 Geometrien) — Estlcam-Element-Modell. Frontend-only (bestehende Endpoints), 4 vitest. | [#56](https://github.com/MadGapun/CAMWOSA/issues/56) | [Per-Feature-Override](Per-Feature-Override.md) | ✅ |
