...
- **v0.0.1-alpha.18** (A49-Kern: Werkstück-Transformation beim Umspannen) — `cam/umspannung.py`: `WerkstueckTransformation` (Spiegeln/Drehen/Wenden um die Werkstück-Mitte) + `transformiere_toolpath` (korrigiert Bogen-Drehrichtung G2↔G3 bei Spiegelung) + Stabilitäts-Heuristik. `Setup.transformation`-Feld, Endpoint `POST /api/operations/transformiere` + MCP `toolpath_transformieren`, UI-Editor pro Setup in WorkflowView. 15 Tests → Backend **909** grün. Damit 2-/N-seitige Bearbeitung koordinaten-korrekt. Offen: Auto-Apply im Multi-Setup-Export + N-Sided-Wizard.
- **v0.0.1-alpha.17** (Q3: Multi-Geometrie pro Operation + Per-Pfad-Override) — eine Operation bearbeitet jetzt **alle** zugewiesenen Geometrien (vorher nur die erste), je Geometrie ein Toolpath → kombiniert (`api/toolpathKombi.ts`). Pro Kontur Feed/Max-Tiefe/Plunge überschreibbar (`geometrie_overrides` + UI `GeometrieFeinabstimmung` ab 2 Geometrien) — Estlcam-Element-Modell, der größte funktionale Wettbewerbs-Delta. Frontend-only, +4 vitest. Damit Cluster Q bis auf Q6 (Workflow-Diagramme) erledigt.
- **v0.0.1-alpha.16** (Cluster Q: Wettbewerbs-Audit + Feinkörnigkeit + Piktogramme) — aus 9-Agenten-Workflow (Estlcam/Carbide/Easel/VCarve/OpenBuilds-Recherche). **Q1** Override-UI-Vollständigkeit (~17 Felder), **Q2** Plunge- vs. Rampen-Feed getrennt (variable Eintauchgeschwindigkeit), **Q4** Operations-Piktogramme (`OperationGrafik`) + Strategie-Piktogramme (`StrategieGrafik`), **Q5** Innen/Außen/Tabs visuell (`KonturSeiteGrafik`), **Q7** QuickStart-Dead-End (#50) gefixt. Backend **893** grün (+12), Frontend +5 Render-Tests. Doku: `docs/ANALYSE-2026-06-05-Wettbewerb-Einstellbarkeit.md`. Offen: Q3 (per-Pfad-Override, Estlcam-Modell), Q6 (Workflow-Diagramme).
- **v0.0.1-alpha.15** (Alles editierbar: Maschinen- + Rotary-Editor) — Markus' Anforderung „man rüstet seine CNC auf — alle Parameter editierbar". **Neuer Maschinen-Editor** (`MaschinenEditor.tsx`: Arbeitsraum, Vorschub/Eilgang, Controller, Sicherheitshöhe, Werkzeugwechsel-Pos, Postprozessor, Spindel-Zuordnung+aktiv, Modi+aktiv) — anlegen/bearbeiten/löschen, data-loss-sicher. **Neuer Rotary-Profil-Editor** (`RotaryProfilEditor.tsx`) + Rotary-Backend-CRUD (vorher nur GET). Damit hat jede Daten-Entität einen UI-Editor (Maschine/Spindel/Werkzeug/Material/Feeds&Speeds/Rohmaterial/Rotary). +4 Tests → 881 grün.
- **v0.0.1-alpha.14** (Spindel-Korrektur + Spindel-Editor) — Markus' reale Spindel (1,5 kW luftgekühlt, VFD, ER11) + SainSmart 710 W in die Bibliothek; ProVerXL-Profil aktive Spindel = VFD (statt Makita). **Neuer Spindel-Editor** (`SpindelEditor.tsx` + Spindel-Bibliothek in MaschinenView: anlegen/bearbeiten/löschen, „aktiv" setzen) — alle Spindel-Werte UI-editierbar. Hochlauf-Dwell folgt der gewählten Spindel. Makita-Referenzen in allen Docs korrigiert. 877 Tests grün.
